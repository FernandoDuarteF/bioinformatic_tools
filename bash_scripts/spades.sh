...
if [[ "$Correction" == 'correct' ]]; then
  spades.py -k 21,33,55,77,99,127 -m 28 --phred-offset 33 --careful -1 $WorkDir/$F_Read -2 $WorkDir/$R_Read -t 16  -o $WorkDir/. --cov-cutoff "$Cutoff"
elif [[ "$Correction" == 'only-assembler' ]]; then
  spades.py -k 21,33,55,77,99,127 -m 28 --phred-offset 33 --careful --only-assembler -1 $WorkDir/$F_Read -2 $WorkDir/$R_Read -t 16  -o $WorkDir/. --cov-cutoff "$Cutoff"
else
